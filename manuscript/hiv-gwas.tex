...
\begin{figure}[H]
	...
	\caption{A high-level schematic of our POUMM-based simulation framework. (A) shows how pathogen genetic effects on spVL evolve along the pathogen phylogeny according to an OU process. (B) shows host genetic effects, which are the sum of independent effects from several causal variants. Each variant can be present in 0, 1, or 2 copies and has some effect $\delta$ on the trait value. We assume each variant has the same magnitude of effect and that half the causal variants have a positive effect and the other half have a negative effect. (C) shows that environmental effects are assumed to be independent between sampled individuals and are drawn from a distribution centered at zero with variance $\sigma^2_e$.}
...
\begin{figure}[H]
	...
	\node[anchor = west] (blabel) at (-1, 4.5) {B. Generating viral effects};
	...
	\node[anchor = west] (clabel) at (-1, 5.55) {C. Generating host effects};
	...
	\node[anchor = west] (dlabel) at (-1, 3) {D. Generating environmental effects};
