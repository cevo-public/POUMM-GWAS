...
\begin{figure}[H]
	\begin{center}
		...
		\caption{A statistical model of the spVL trait. (A) The virus-determined part of spVL, $g_v$, evolves according to an OU process along (C) the viral phylogeny. The dashed line in (A) shows the expected value of the trait at each point in time. (B) The environmentally-determined part of spVL, $e$, including host genetic effects, interaction effects, and other environmental effects, is modeled as a Gaussian random variable whose value is independently drawn for each host at the tips of the phylogeny.}
		\label{fig:spVLModel}
	\end{center}
\end{figure}
