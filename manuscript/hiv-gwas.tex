...
Here we show the root mean square error (RMSE) of $\bm{z} - \bm{\bar{z}}$ as an estimate for the host part of the trait, denoted $\bm{h}$, should be $\approx$ 0.74 in our simulation scheme. First we write the expression for the RMSE:

\begin{equation}
    RMSE = \sqrt{\frac{\sum_i^N(z - \bar{z} - h)^2}{N}}
\end{equation}

Note that under our simulation setup, $z - \bar{z}$ differs from $h$ due to the pathogen effect $g$ and the environmental effect $e$. So the term inside the square root equals the combined variance of these two effects:
...
\begin{tabularx}{\linewidth}{p{1.8cm}p{2cm}p{1.8cm}p{2cm}p{1.8cm}p{2cm}p{1.8cm}} % {lrrrrrr} {llclcccc}
\hline
