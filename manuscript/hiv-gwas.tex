...
\usepackage[backend=bibtex,style=nature]{biblatex} % Borrowing citation formatting from Nature
...
Variation in spVL comes from several sources: direct host genetic effects, pathogen genetic effects, interaction effects between host and pathogen genetic factors, and other environmental effects. Of these, only pathogen genetic effects are heritable from one transmission partner to another \parencite{Leventhal2016}. To characterize these effects, we use a phylogenetic mixed model (PMM) \cite{Housworth2004}. PMMs assume continuous traits like spVL are the sum of independent heritable and non-heritable parts. In our case, pathogen genetic effects comprise the heritable part and all other effects comprise the non-heritable part. Then, the heritable part is modeled by a random process occurring in continuous time along the branches of the pathogen phylogeny. The non-heritable part is modeled as Gaussian noise added to sampled individuals at the tips of the phylogeny (Figure \ref{fig:spVLModel}).
...
So far, PMMs with two types of random processes have been used to model spVL evolution. The Brownian Motion (BM) process assumes unbounded trait values, i.e. spVL can attain any value. The Ornstein-Uhlenbeck (OU) process assumes trait values fluctuate around an optimal value, i.e. extreme spVL values are unlikely. \cite{Mitov2018} and \cite{Bertels2018} previously showed the OU process has higher statistical support for spVL. This makes sense given that spVL is likely under stabilizing selection to maximize viral transmission potential \cite{Fraser2014}. Therefore, we assume the OU process. The full model is called the phylogenetic Ornstein-Uhlenbeck mixed model (POUMM) and is described in detail by \cite{Mitov2018}. Here, we review the main relevant points in the spVL context.

Under the POUMM, the spVL trait $z$ is the sum of viral genetic efects $g_v$, host genetic effects $g_h$, and other environmental or interaction effects $\epsilon$. We can group the non-heritable effects $g_h$ and $\epsilon$ into a broader category of ``environmental'' effects $e$: 
\begin{equation}
    z = g_{v} + e
\end{equation}
...
\end{align}
	...
	Equations \ref{eq:MuG} and \ref{eq:SigmaG} follow from eq. \ref{eq:pdfG} and eq. 371, p. 42, section 8.1.8 ``Product of Gaussian densities'' in \cite{Petersen2012}.
\end{proof}
...
We propose to improve standard GWAS for infectious diseases by estimating and removing trait variability due to pathogen genetic effects. Our new framework is as follows:
...
\begin{enumerate}
	...
	\item Estimate the parameters of the POUMM based on the trait values and the pathogen phylogeny. This can be done with e.g. the R package POUMM \cite{Mitov2017a-POUMM}.
...
We compared our proposed GWAS framework with a more standard approach by performing two different GWAS on the same SHCS human genotypes. In the ``GWAS with standard trait value'' we used the unmodified trait value, our calculated spVL values, as the GWAS response variable. In the ``GWAS with estimated non-pathogen part of trait'' we used our estimates for the non-pathogen effects on spVL. Figure \ref{fig:gwas-results}A shows that results are qualitatively similar between the two GWAS. Q-Q plots show the distribution of p-values are very similar as well (figure \ref{fig:qq-plots}). Figure \ref{fig:gwas-results}B shows how the strength of association changed for some variants in the MHC and \emph{CCR5} regions. Taking into account phylogenetic information slightly decreased association strength for most variants in \emph{CCR5} region. Association strength increased for some variants in the MHC, for example, SNP rs9265880 had the greatest increase in significance in the MHC region, from a p-value of $3.5 \times 10^{-07}$ to $7.7 \times 10^{-09}$. However, the top-associated variants in the MHC and \emph{CCR5} regions were consistent regardless of the GWAS response variable used \ref{tab:gwas-top-associations}. For comparison, table \ref{tab:comp-gwas-mclaren-snps} shows our GWAS results for the two top-associated SNPs identified by \cite{McLaren2015}. In summary, there are no clear patterns that point to new regions of association in the human genome with spVL when we take into account the pathogen phylogeny.

% TODO
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.7\linewidth]{figures/gwas_results}
		\caption{Results from comparative GWAS. (A) shows association p-values for the same host variants from the SHCS cohort in GWAS with two different response variables. On the left, we used unmodified spVL values. On the right, we used our estimates for the non-pathogen effects on spVL. The alternating colors correspond to different chromosomes. (B) compares the strength of association for variants in the \emph{CCR5} and MHC regions between the two GWAS (positions 45.4 - 47Mb on chromosome 3 and 29.5 - 33.5Mb on chromosome 6 for the \emph{CCR5} and MHC, respectively). Base positions are with reference to genome build GRCh37. The color of each point represents the difference in -log$_{10}$ p-value between the two GWAS. Red means taking into account phylogenetic information decreased the strength of association and blue means it increased it. The dashed lines show genome-wide significance at p = 5x10$^{-8}$).}
...
\begin{table}[H]
\centering
\caption{Top association results from \cite{McLaren2015} compared to results from this study from GWAS with unmodified (standard) spVL values and with our estimates for the non-pathogen effects on spVL.}
...
We present a novel, phylogeny-aware GWAS framework to correct for heritable pathogen effects on infectious disease traits.  

% Contextualize the study
We estimated HIV-1 spVL heritability to be 45\% (95\% HPD 24 - 67\%) in the SHCS. Our estimate is at the high end of previous estimates based on the POUMM in European cohorts, but the uncertainty intervals largely overlap: \cite{Bertels2018} estimated 29\% (N = 2014, CI 12 - 46\%) also from the SHCS and \cite{Blanquart2017} estimated 31\% (N = 2028, CI 15 - 43\%) from a pan-European cohort. Our sample size (N = 1493 individuals) is quite a bit smaller than these other studies, possibly because we restrict samples based on having $pol$ gene sequences with at least 750 non-ambiguous bases. Here our aim was to reconstruct a high-quality phylogeny, since the POUMM does not account for phylogenetic uncertainty. 

% State some limitations
Given our estimates for the heritability of spVL and the selection strength on this trait, our simulation results show that we cannot expect a significant improvement in GWAS power to identify host genetic determinants of this trait (figure \ref{fig:simulationResults}). So although there is a growing consensus that spVL is highly heritable (see \cite{Mitov2018} and citations therein), this does not necessarily mean individuals in a GWAS cohort have phylogenetically correlated spVL values.  

% Summarize the paper
% Make recommendations or suggestions for future research
\begin{itemize}
...
\begin{tabular}{lllP{2.2cm}P{2.2cm}P{1.6cm}P{2.2cm}}
  ...
  \hline
   ...
   \hline
\end{tabular}
\label{gwas-top-associations}
\end{table}


% \begin{longtable}[H]{lp{1cm}p{2cm}p{1cm}p{2cm}p{1cm}p{2cm}} % {lrrrrrr}{lrrrrrr}
% 		\caption{Association results for common class I HLA alleles ranked by p-value from the GWAS by \cite{McLaren2015} compared to results from the current study.}
% 		\hline
...
% 	\end{equation}
% 	
% 	Equations \ref{eqPdfGValues1} and \ref{eqMuSigmaGValues} follow from eq. \ref{eqPdfGValues2} and eq. 371, p. 42, section 8.1.8 ``Product of Gaussian densities'' in \cite{Petersen2012}.
% 	
% \end{proof}
%
...
%	& \multicolumn{2}{c}{\specialcell[l]{\cite{McLaren2015} study \\ N = 6,315}} & \multicolumn{3}{c}{\specialcell[l]{Current study \\ N = 1,370}} & \\ \midrule 
...
%			\hline
%		\end{tabular}
%		\caption{Top association results from \cite{McLaren2015} compared to association results for the best proxy SNPs available in the current study. Proxy SNPs were chosen based on linkage disequilibrium (as measured by R$^2$) with the McLaren et al. SNPs.}
%	\end{center}
%\end{table}

% Again following the procedure of \cite{McLaren2015}, we imputed common HLA genotypes to more finely map association signals in the MHC region. Some HLA alleles also became more significant with a phylogenetic correction (table \ref{tab:lim-mclaren-comparison}).
