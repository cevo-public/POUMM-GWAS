...
In this work, we draw from the field of phylogenetic comparative methods to propose a new two-step framework  which corrects for pathogen population structure and thus satisfies the GWAS assumption of independent samples. 
...
\includegraphics[width = \linewidth]{figures/application_experimental_setups.pdf}
	\caption{A high-level schematic of the experimental setup for the two application datasets. For application (A) to HIV-1 spVL in the Swiss HIV Cohort Study, data are paired viral and human genotypes and associated spVL measurements. We fit the POUMM to the viral phylogeny and spVL values associated with each infected individual ($z_1$, $z_2$, ..., $z_{1493}$). For application (B) to \emph{A. thaliana}-\emph{X. Arboricola} QDR from \cite{Wang2018Two-wayGenomes}, data are bacterial and plant genotypes with QDR measurements for all possible combinations of pathogen and host plant strains. We fit the POUMM to the bacterial phylogeny and mean QDR calculated for each pathogen strain across all the hosts plant types ($\bar{z}_1$, $\bar{z}_2$, ..., $\bar{z}_{22}$).}
...
\begin{figure}[H]
	\begin{center}
		...
		\caption{Results from comparative GWAS on HIV-1 spVL data. (A) shows association p-values for the same host variants from the SHCS cohort in GWAS with two different response variables. On the left, we used unmodified (total) spVL values. On the right, we used our estimates for the non-pathogen effects on spVL. The alternating shades correspond to different chromosomes. (B) compares the strength of association for variants in the \emph{CCR5} and MHC regions between the two GWAS (positions 45.4 - 47Mb on chromosome 3 and 29.5 - 33.5Mb on chromosome 6 for the \emph{CCR5} and MHC, respectively). Base positions are with reference to genome build GRCh37. The color of each point represents the difference in -log$_{10}$ p-value between the two GWAS. Red means taking into account phylogenetic information decreased the strength of association and blue means it increased it. The dashed lines show genome-wide significance at p = $5 \times 10^{-8}$.}
...
% Summarize the paper
In summary, we present a coherent infectious disease GWAS framework which takes the pathogen phylogeny into account when searching for host determinants of a disease trait. We further show that the pathogen phylogeny only has an impact on the GWAS outputs if heritability of the trait amongst infection partners $>$ 25\%.
This means that for the systems studied here, spVL in HIV and QDR for X. arboricola infections in A. thaliana, the phylogeny correction does not change GWAS results and thus previously published results are not biased by the pathogen phylogeny.
...
Our method assumes the phylogeny accurately reflects the evolutionary relationships between pathogen strains. Previously, \cite{Hodcroft2014} observed HIV spVL heritability estimates based on $pol$ gene sequences were robust to including or not including resistance-associated codons. Our analysis includes these codons. For the HIV application, we additionally tested the sensitivity of the inference to phylogenetic uncertainty. We inferred the phylogeny again, this time using the IQ-TREE option ``wt'' to output all locally optimal trees. We fit the POUMM to two randomly selected trees from this set. Posterior mean parameter estimates were within the 95\% HPD intervals inferred from the maximum likelihood tree. GWAS results were also comparable. 
...
\begin{figure}[H]
\begin{center}
\includegraphics[width = \linewidth]{figures/gwas_results_xanthomonas.png}
	\caption{Results from comparative GWAS on \emph{A. thaliana}-\emph{X. arboricola} QDR data. The two facets show association p-values for the same host \emph{A. thaliana} variants in GWAS with two different response variables. On the left, we used unmodified (total) QDR values. On the right, we used our estimates for the non-pathogen effects on QDR. In this case, these values are total QDR minus a pathogen strain-specific scaled mean QDR. The alternating shades correspond to different chromosomes. The dashed lines show significance at significance level 0.05 with a Bonferroni correction for multiple testing.}
...
\begin{table}[H]
\caption{Number of variants for \emph{A. thaliana} QDR GWAS after sequential filtering steps.}
	\begin{tabularx}{\linewidth}{XX}
	\hline
	Variant filter & Number of variants remaining \\
	\hline 
	...
	NA p-value due to too-high correlation with a covariate & 1070541 \hline			
	\end{tabularx}
...
\begin{enumerate}
    ...
    \textcolor{blue}{This is correct, our method assumes the phylogeny accurately reflects the evolutionary history of the pathogen strains. We tested the sensitivity of the method to phylogenetic uncertainty in the HIV application. This is described in the section ``'': ``''.}
