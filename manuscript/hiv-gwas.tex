...
\begin{equation}
	H^2_{\bar{t}} = \frac{Var[g(t)]}{Var[g(t)] + Var(\epsilon)} = \frac{\frac{\sigma^2}{2\alpha}(1 - e^{-2\alpha t})}{\frac{\sigma^2}{2\alpha}(1 - e^{-2\alpha t}) + \sigma^2_\epsilon}
	\label{eq:POUMM-H2}
\end{equation}
...
\begin{figure}[H]
	\begin{center}
		...
		\caption{Example simulated data from two evolutionary scenarios where (left) a phylogenetic correction to trait values does not improve GWAS power and (right) where it does. These examples correspond to the empty points in Figure \ref{fig:simulationResults}. Apparent is that the phylogenetically-corrected trait values (y-axis) correspond better with the true host part of the trait when trait heritability due to the pathogen is higher (right side). Selection strength was fixed to 0.1 time $^{-1}$ for both scenarios and all other parameters were fixed as in the full simulation study. }
...
\begin{table}[H]
	...
	\begin{tabularx}{\linewidth}{p{1.5cm}ll} \toprule 
		...
		$\sigma$ & $\sigma = \sqrt{\frac{-2\alpha H^2 \sigma^2_z (0.75 - H^2)}{(H^2 - 1)(1 - exp(-2\alpha\bar{t}))}}$ & Time-unit standard deviation of OU process \\ 
