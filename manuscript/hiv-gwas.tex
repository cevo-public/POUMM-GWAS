...
\includegraphics[width = 0.462\linewidth]{figures/trait_values_arboricola.png}
	\caption{A comparison of measured (calculated) QDR values versus our estimated non-pathogen effect on QDR for each selected \emph{A. thaliana} host - \emph{X. arboricola} pathogen pairing selected for the GWAS. The histograms show the marginal distribution of each value across the individual pairings.}
...
\begin{figure}[H]
	...
	\caption{Results from comparative GWAS on \emph{A. thaliana}-\emph{X. arboricola} QDR. Points are association p-values for the same \emph{A. thaliana} host variants in GWAS with two different response variables. On the left, we used unmodified (total) QDR values. On the right, we used our estimates for the non-pathogen effects on QDR. The alternating shades correspond to different chromosomes. The dashed lines show significance at $\alpha$ = 0.05 with a Bonferroni correction for multiple-testing.}
