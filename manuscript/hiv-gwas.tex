...
\begin{figure}[H]
	...
	\caption{A high-level schematic of our POUMM-based simulation framework. (A) shows how pathogen genetic effects on spVL evolve along the pathogen phylogeny according to an OU process. (B) shows host genetic effects, which are the sum of independent effects from several causal variants. Each variant can be present in 0, 1, or 2 copies and we assume half have effect size $\delta$ on the trait value and half have effect size $-\delta$. (C) shows that environmental effects are assumed to be independent between sampled individuals and are drawn from a distribution centered at zero.}
