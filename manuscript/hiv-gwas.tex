...
% The gap
Most of the GWAS for human genetic determinants of spVL conducted so far \citep{Dalmasso2008, Fellay2007, Pereyra2010, Fellay2009, Pelak2010, VanManen2011, McLaren2012, McLaren2015} do not explicitly consider any viral effect on spVL. In these GWAS, viral genetic effects are lumped in with residual variance due to other, non-genetic factors. This has several potential negative consequences. Viral effects may be (i) confounding or (ii) extraneous variables that produce distorted or spurious associations between host genetic variants and a disease trait in host GWAS. 
...
For example, stratification in both the host and pathogen populations may yield spurious correlations in host GWAS that are really due to independent pathogen factors. Then, co-evolution between a pathogen and its host may yield distorted estimates for host genetic effects that are not broadly protective against all pathogen strains. Finally, (iii) spVL values from a cohort are not truly independent samples, given that patients closer in the transmission chain have more similar strains and therefore more similar spVL values.
...
\begin{figure}[H]
	\begin{center}
		...
		\caption{Example simulated data from two evolutionary scenarios where (left) a phylogenetic correction does not improve GWAS power and (right) where it does. The only difference in the two simulations is heritability; selection strength was fixed to 0.1 time $^{-1}$ and all other parameters were fixed as in the full simulation study. These examples correspond to the empty points in Figure \ref{fig:simulationResults}.}
...
\emph{A. thaliana} genotypes were downloaded from the 1001 Genomes project \citep{Alonso-Blanco20161135Thaliana}. Quantitative diseases resistance (QDR) score measurements and \emph{X. arboricola} genotypes were provided by the authors of \citep{Wang2018Two-wayGenomes}. The \emph{X. arboricola} QDR trait values were generated in a fully-crossed experimental design, where each host type (accession) was infected with each pathogen strain. QDR was measured on 4 leaves for each host-pathogen pairing. 

For phylogenetic inference, the \emph{X. arboricola} genotype matrix consisted of allele dosage information for 33,610 SNPs in 22 strains. We used the ``dist.gene'' and ``nj'' functions from the ape package in R to construct a pairwise genetic distance matrix and then a neighbor-joining tree from these data. We calculated mean QDR across all leaves infected on all hosts for each \emph{X. arboricola} strain.

For GWAS, we retained data from 133 \emph{A. thaliana} accessions with associated QDR measurements.
...
We corrected HIV-1 spVL and \emph{X. arboricola} QDR trait values using the method described in this paper. For each host individual, we estimated the pathogen part of their trait value using equation \ref{eq:MuG} and the corresponding non-pathogen part using equation \ref{eq:EHat}. For the POUMM parameters $\alpha$, $\sigma$, $\theta$, and $\sigma_e$, we used the posterior mean estimates generated as described above. For the \emph{A. thaliana}-\emph{X. arboricola} trait correction and downstream GWAS, we randomly selected one pathogen pairing for each host to analyze. This makes the dataset analogous to the observational HIV-1 dataset. All the code used to implement the method is available at https://github.com/cevo-public/POUMM-GWAS.
...
\caption{Number of variants for \emph{X. arboricola} QDR GWAS after sequential filtering steps.}
	...
	\hline 
	Raw data & 12883854 \\
	Bi-allelic variants & 11769920 \\ \hline			
	\end{tabularx}
