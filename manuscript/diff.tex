...
%DIF DEL manuscript/mbe-submission/hiv-gwas-original-submission.tex   Tue Jun 14 14:26:47 2022
%DIF ADD manuscript/hiv-gwas.tex                                      Tue Jun 14 14:24:50 2022
...
\DIFaddend \begin{equation}
	...
	\label{eq:OUcov}
\end{equation}
Next, we remember that \DIFdelbegin \DIFdel{$e$ }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon$ }\DIFaddend is the non-heritable \DIFdelbegin \DIFdel{, environmental part of spVL. $e$ }\DIFdelend \DIFaddbegin \DIFadd{part of the trait. $\epsilon$ }\DIFaddend is modeled as a Gaussian random variable that is time- and phylogeny-independent. The expectation of \DIFdelbegin \DIFdel{$e$ }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon$ }\DIFaddend is 0, meaning environmental effects are equally likely to raise or lower \DIFdelbegin \DIFdel{spVL from the virus-determined }\DIFdelend \DIFaddbegin \DIFadd{the trait from the pathogen-determined }\DIFaddend level. The parameter \DIFdelbegin \DIFdel{$\sigma_e^2$ }\DIFdelend \DIFaddbegin \DIFadd{$\sigma_\epsilon^2$ }\DIFaddend measures the between-host variance of the environmental effect.
\DIFaddbegin 
...
Importantly, equation \ref{eq:MuG} is the maximum likelihood estimate for $\bm{g}_v$, the \DIFdelbegin \DIFdel{viral effect on spVL}\DIFdelend \DIFaddbegin \DIFadd{pathogen effect on the trait}\DIFaddend , taking into account all available information - measured \DIFdelbegin \DIFdel{spVL}\DIFdelend \DIFaddbegin \DIFadd{trait values}\DIFaddend , the pathogen phylogeny, and inferred POUMM parameters. This estimator is an inverse-variance weighted average of measured \DIFdelbegin \DIFdel{spVL }\DIFdelend \DIFaddbegin \DIFadd{trait }\DIFaddend ($\bm{z}$) and information from the POUMM evolutionary model ($\bm{\mu}_{OU}$). In other words, \DIFdelbegin \DIFdel{$g_v$ }\DIFdelend \DIFaddbegin \DIFadd{$g$ }\DIFaddend will be closer to \DIFdelbegin \DIFdel{measured spVL if spVL }\DIFdelend \DIFaddbegin \DIFadd{the measured trait value if the trait }\DIFaddend is not very heritable. If \DIFdelbegin \DIFdel{spVL }\DIFdelend \DIFaddbegin \DIFadd{the trait }\DIFaddend is highly heritable, \DIFdelbegin \DIFdel{$g_v$ }\DIFdelend \DIFaddbegin \DIFadd{$g$ }\DIFaddend will be closer to the expected value under the POUMM, i.e. take more information from the phylogenetic relationships between infecting strains. 
...
First, we evaluated how well our method estimated the additive host genetic effects from the simulated data. Additive host genetic effects represent an ideal (albeit unattainable) baseline for infectious disease GWAS. Figure \ref{fig:simulationResults}A shows that our method incorporating phylogenetic information can more accurately estimate \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend value compared to the trait value. To ensure a fair comparison, we scaled trait values to have the same mean, zero, as host genetic effects so as not to bias the root mean squared error (RMSE) by a constant factor. \DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{As shown in }\DIFaddend the supplemental material, we \DIFdelbegin \DIFdel{show why the }\DIFdelend \DIFaddbegin \DIFadd{can calculate the expected RMSE using the }\DIFaddend scaled trait value \DIFdelbegin \DIFdel{is expected to have an RMSE of approximately }\DIFdelend \DIFaddbegin \DIFadd{across scenarios in our simulation scheme because the variance in the trait due to viral genetic effects and environmental effects is fixed. Thus, we expect the RMSE using the scaled trait value to be }\DIFaddend 0.74 \DIFdelbegin \DIFdel{under our simulation scheme}\DIFdelend \DIFaddbegin \DIFadd{across all simulation scenarios}\DIFaddend . By incorporating phylogenetic information, we can improve upon this error in scenarios where the trait is highly heritable\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend under low selection pressure\DIFdelbegin \DIFdel{. This is because high heritability means the pathogen greatly effects the trait value. Then, when the trait is under weak selection, pathogen effects can drift far from the long-term optimum and stochastic fluctuations are low to maintain the same heritability. Thus, our method performs well }\DIFdelend \DIFaddbegin \DIFadd{, and with relatively moderate stochastic fluctuations compared to outbreak duration. Figure \ref{fig:simulationTraitExample} gives some intuition for how this correction works by contrasting simulated scenarios with high and low heritability and low selection strength/ low stochastic fluctuations. These parameter combinations mean trait values are phylogentically correlated at clustered tips in the phylogeny. This information is the basis from which our method estimates the pathogen and corresponding non-pathogen effects on the trait. Figure \ref{fig:tip-trait-correlation} shows in more detail how trait values are correlated at closely clustered tips }\DIFaddend when an infectious disease trait is highly heritable and \DIFdelbegin \DIFdel{trait values are highly correlated amongst closely related members of a transmission chain, but vary widely within the entire cohort}\DIFdelend \DIFaddbegin \DIFadd{selection strength is low/ stochastic fluctuations are low}\DIFaddend .
...
\includegraphics[width = \linewidth]{figures/tip_trait_correlation.png}
	\caption{\DIFaddFL{Correlations between trait values in pairs of tips in four simulated scenarios. These examples correspond to the four empty points in Figure \ref{fig:simulationResults}. Correlations are calculated for pairs of tips binned by phylogenetic distance (into deciles) across the 20 replicate simulations for each of the four evolutionary scenarios. Trait values are only noticeably correlated for closely clustered tips under the scenario with high pathogen heritability and low selection strength/ low stochastic fluctuations (upper left facet).}}
...
\cite{Wang2018Two-wayGenomes} }\hskip0pt%DIFAUXCMD
used a linear mixed model in which the experimental unit is QDR scored on individual leaves, whereas our estimate is based on much coarser binning of QDR scores into a mean score across all leaves on all host accessions and all replicates (N = 22). Furthermore, the QDR score trait values were not truly continuous (scores were measured on an integer scale from 0 to 4). Thus, these data partially violate the assumptions of the POUMM. Compared to HIV-1, we estimate very low selection strength for virulence in }\emph{\DIFadd{X. arboricola}}\DIFadd{. As \mbox{%DIFAUXCMD
\cite{Wang2018Two-wayGenomes} }\hskip0pt%DIFAUXCMD
explain, }\emph{\DIFadd{X. arboricola}} \DIFadd{strains with differing virulence can co-inhabit populations of }\emph{\DIFadd{A. thaliana}}\DIFadd{. This might also point to low selection on }\emph{\DIFadd{X. arboricola}} \DIFadd{virulence. Furthermore, expected correlation in virulence between related strains of }\emph{\DIFadd{X. arboricola}} \DIFadd{was smaller than for HIV-1.
}

\DIFadd{The }\DIFaddend main novelty of our approach \DIFdelbegin \DIFdel{was to correct the spVL trait }\DIFdelend \DIFaddbegin \DIFadd{is to use these POUMM parameter estimates to correct infectious disease trait values }\DIFaddend prior to performing \DIFdelbegin \DIFdel{the }\DIFdelend GWAS, thereby estimating and removing pathogen effects. In simulations, we show that when trait heritability amongst infection partners is greater than approximately 25\%, GWAS power to detect host genetic variants associated with the same trait is reduced. Our method can correct for this effect in certain evolutionary scenarios by using information from the full pathogen phylogeny. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Based on our simulation results, our method is anticipated to be very useful for disease traits that are highly heritable from donor to recipient and  maintain a high correlation between sampled individuals. In simulations, we showed this is the case when \DIFaddbegin \DIFadd{pathogen }\DIFaddend heritability is high, selection strength is low, and trait values are not subject to strong stochastic fluctuations. \DIFdelbegin \DIFdel{So, }\DIFdelend \DIFaddbegin \DIFadd{It is possible that traits under strong selection pressure would also be highly heritable and phylogenetically correlated, for instance antimicrobial resistance. However, such traits might violate the POUMM assumption that trait values vary as a random walk if they are caused by few mutations of strong affect. In summary, }\DIFaddend cohort-level, phylogenetically structured differences in the measured trait value are necessary for our approach to outperform state of the art methods. 

Given our estimates for \DIFdelbegin \DIFdel{the heritability of spVL and the }\DIFdelend \DIFaddbegin \DIFadd{trait heritability and }\DIFaddend selection strength on \DIFdelbegin \DIFdel{this trait using Swiss HIV cohort data}\DIFdelend \DIFaddbegin \DIFadd{HIV-1 spVL and }\emph{\DIFadd{A. thaliana}} \DIFadd{quantitative disease resistance to }\emph{\DIFadd{X. arboricola}}\DIFaddend , our simulation results reveal that we cannot expect a significant improvement in GWAS power \DIFdelbegin \DIFdel{for human genetic determinants of spVL }\DIFdelend (Figure \ref{fig:simulationResults}). \DIFdelbegin \DIFdel{Indeed the results of our  GWAS method presented here and a standard GWAS are very similar. Future applications might investigate other clinically significant disease traits and outcomes that are affected by both host and pathogen genetic factors, for instance Hepatitis B Virus-related hepatocellular carcinoma \mbox{%DIFAUXCMD
...
, and susceptibility to or severity of certain bacterial infections, e.
g. \mbox{%DIFAUXCMD
\cite{Messina2016ImpactInfections, Donnenberg2015BacterialStudies}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{Certain pairs of samples in the HIV-1 cohort were indeed expected to have phylogenetically correlated spvL values (maximum expected correlation between any two samples was 0.45). However, the overall effect on GWAS is small. For HIV-1 spVL, our phylogenetic correction slightly decreases p-values for variants in $CCR5$ and slightly decreases some and increases other p-values for variants in the MHC (Figure \ref{fig:gwas-results}B). Simulations show we shouldn't expect a net p-value decrease, but our simulations represent an ideal scenario since we simulate under the POUMM. In real life, un-modeled evolutionary pressures like drug treatment and host-specific HLA alleles might cause the reduced p-values. However, the overall picture is consistent between the two GWAS (Figure \ref{fig:gwas-results}A). Therefore, we conclude that GWAS for host determinants of HIV-1 subtype B spVL is robust to our correction for pathogen effects. For }\emph{\DIFadd{A. thaliana}} \DIFadd{QDR, inferred phylogenetic correlations are too weak to justify our phylogenetic correction (maximum expected correlation between strains was $3.2 \times 10^{-12}$). \mbox{%DIFAUXCMD
\cite{Wang2018Two-wayGenomes} }\hskip0pt%DIFAUXCMD
found no significant }\emph{\DIFadd{A. thaliana}} \DIFadd{variants associated with QDR using a linear mixed model jointly accounting for host genetic effects, pathogen genetic effects, and interaction effects. As with HIV-1 spVL, our results do not challenge this previous finding.
}\DIFaddend 

Our \DIFdelbegin \DIFdel{method is convenient for GWAS because it is simply a }\DIFdelend \DIFaddbegin \DIFadd{approach is envisioned to be complementary to previously developed methods for infectious disease GWAS. First, it provides additional information on the evolutionary dynamics of the trait in the pathogen population. Then, it is a convenient }\DIFaddend pre-processing step \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{for GWAS because it simply }\DIFaddend produces an alternate response variable for GWAS association tests. \DIFdelbegin \DIFdel{Therefore, }\DIFdelend \DIFaddbegin \DIFadd{As in the }\emph{\DIFadd{A. thaliana}}\DIFadd{-}\emph{\DIFadd{X. arboricola}} \DIFadd{application, fitting the POUMM may reveal that expected phylogenetic correlations between samples are not strong enough to justify using our method to correct trait values in a GWAS. In this case, one may wish to use a linear mixed model as in \mbox{%DIFAUXCMD
\citep{Wang2018Two-wayGenomes}}\hskip0pt%DIFAUXCMD
, where the pathogen effect is co-estimated as a random effect. In cases where a correction can be estimated and applied, }\DIFaddend it is still possible to use \DIFdelbegin \DIFdel{previously developed, well-documented, and fast tools }\DIFdelend \DIFaddbegin \DIFadd{any of the number of previously developed GWAS models }\DIFaddend for the actual association testing (we used \DIFdelbegin \DIFdel{PLINK \mbox{%DIFAUXCMD
...
\emph{\DIFadd{A. thaliana}} \DIFadd{and }\emph{\DIFadd{X. arboricola}} \DIFadd{genotyping and quantitative disease resistance (QDR) measurements are described in detail in \mbox{%DIFAUXCMD
\cite{Wang2018Two-wayGenomes}}\hskip0pt%DIFAUXCMD
...
\DIFdelendFL \DIFaddbeginFL \centering
	...
	\caption{\DIFaddFL{Quartile-quartile plots from association tests. The dashed green line shows the y = x line.}}
	\label{fig:qq-plots}
...
%DIFDELCMD < 	\end{center}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{figures/host_genotype_pca.png}
...
\begin{figure}[H]
\begin{center}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = \linewidth]{figures/spvl_on_tree.png}
%DIFDELCMD < 	\caption{%%%
\DIFdelFL{Inferred HIV-1 $pol$ gene phylogeny with tips colored by (A) calculated spVL, (B) estimated non-pathogen effects on spVL and (C) estimated pathogen effects on spVL.}%DIFDELCMD < \MBLOCKRIGHTBRACE
%DIFDELCMD < 	\label{fig:spvl-on-tree}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = \linewidth]{figures/qdr_on_tree_arboricola.png}
...
\DIFdelendFL \centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth]{figures/qq_plots.png}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.7\linewidth]{figures/poumm_parameter_estimates_arboricola.png}
		\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Quartile-quartile plots from association tests}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Posterior distributions compared to the prior for POUMM parameter estimates based on }\emph{\DIFaddFL{A. thaliana}}\DIFaddFL{-}\emph{\DIFaddFL{X. arboricola}} \DIFaddFL{data}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{The dashed green line shows }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{We ran two different MCMC chains to ensure }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{y = x line}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{estimates converged}\DIFaddendFL .}
\DIFdelbeginFL %DIFDELCMD < \label{fig:qq-plots}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:poumm-parameters-qdr}
\DIFaddendFL \end{figure}
...
\begin{table}[H]
\caption{POUMM parameter estimates for \DIFdelbeginFL \DIFdelFL{spVL from previous studies}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{QDR based on }\emph{\DIFaddFL{A. thaliana}}\DIFaddFL{-}\emph{\DIFaddFL{X. arboricola}} \DIFaddFL{data data}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{HPD = Highest posterior density.}\DIFaddendFL }
	\DIFdelbeginFL %DIFDELCMD < \begin{tabularx}{\linewidth}{llXXX}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \begin{tabularx}{\linewidth}{XXX} \DIFaddendFL \hline 
	Parameter & \DIFdelbeginFL \DIFdelFL{Value (Uncertainty) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Posterior mean }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{Reference }%DIFDELCMD < & %%%
\DIFdelFL{Notes }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{95\% HPD }\DIFaddendFL \\ \hline 
	\DIFdelbeginFL \DIFdelFL{$v_0$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{${g_{0}}$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{5.54 (4.04 - 7.25) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.40 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\citet{Mitov2018} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{8}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(0.01}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{483 UK HIV cohort individuals, $pol$ tree }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.78) }\DIFaddendFL \\
	$\theta$ & \DIFdelbeginFL \DIFdelFL{4.45 (4.41 - 4.49) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.39 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\citet{Mitov2018} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & \\
%DIFDELCMD < 	%%%
\DIFdelFL{$\theta$ }%DIFDELCMD < &  %%%
\DIFdelFL{4.0 }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{1.6 - 4.)) }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\citet{Bertels2018} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{3,036 SHCS individuals, $pol$ tree }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.30, 0.49) }\DIFaddendFL \\
	\DIFdelbeginFL \DIFdelFL{$\theta$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\sigma$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{4.1 (3.5 - 4.9) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.03 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\citet{Blanquart2017} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{1, 581 subtype B individuals from Europe, whole genome tree }%DIFDELCMD < \\
%DIFDELCMD < 	%%%
\DIFdelFL{$\alpha$ }%DIFDELCMD < & %%%
\DIFdelFL{28.78 }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{16.64 - 46.93) }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\citet{Mitov2018} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0, 0.06) }\DIFaddendFL \\
	$\alpha$ & \DIFdelbeginFL \DIFdelFL{32.7 (}\DIFdelendFL 0.03 \DIFdelbeginFL \DIFdelFL{- 57.6) }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\citet{Bertels2018} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & \\
%DIFDELCMD < 	%%%
\DIFdelFL{$\alpha$ }%DIFDELCMD < & %%%
...
\DIFdelFL{2.97 }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{1.95 - 4.37) }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\citet{Mitov2018} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0, 0.05) }\DIFaddendFL \\
	\DIFdelbeginFL \DIFdelFL{$\sigma$ }%DIFDELCMD < & %%%
\DIFdelFL{1.3 (0.66 -1.87) }%DIFDELCMD < &  %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\citet{Blanquart2017} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & \\
%DIFDELCMD < 	%%%
\DIFdelendFL $\sigma_e$ & \DIFdelbeginFL \DIFdelFL{0.77 (0.73, 0.8) }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\citet{Mitov2018} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.18 }& \DIFaddFL{(0.08, 0.27) }\DIFaddendFL \\ 
	\DIFdelbeginFL \DIFdelFL{$\sigma_e$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$H^2_{\bar{t}}$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.61 (0.54, 0.65) }%DIFDELCMD < &  %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\citet{Blanquart2017} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.33 }& \DIFaddFL{(0.0, 0.77) }\DIFaddendFL \\ \hline
	\end{tabularx}
	\DIFdelbeginFL %DIFDELCMD < \label{tab:POUMMparams-prev-estimated}
%DIFDELCMD < %%%
...
%DIFDELCMD < %%%
\DIFdelend % latex table generated in R 4.1.1 by xtable 1.8-4 package
