...
Load data
```{r}
accuracy_simulation_results <- read.delim("output_revisions/2022-06-11_estimator_accuracy_MLE.txt")
tpr_simulation_results <- read.delim("output_revisions/2022-06-11_GWAS_TPR_MLE.txt")
...
example_data1 <- data.frame(
        ...
        alpha = get_xval_for_sim_plots(real_vals = c(0.1, 0.5, 1, 5, 10, 20, 30, 40, 50, 60), estimate = 5))
...
rmse_plot <- plotHeatmap(
        ...
        geom_point(data = example_data1, fill = "black", shape = 1) + 
        geom_point(data = example_data2, fill = "black", shape = 1)
...
top_row <- plot_grid(NULL, rmse_plot, rel_widths = c(1.03, 3), labels = c("", "A"))
