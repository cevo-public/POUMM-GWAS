...
min_runID <- min(example_data_lo_h2$runID)
example_data_lo_h2 <- example_data_lo_h2 %>% filter(runID == min_runID)
example_data_hi_h2 <- example_simulation_results %>% filter(H2 == 0.75, alpha == 0.1)
min_runID <- min(example_data_hi_h2$runID)
example_data_hi_h2 <- example_data_hi_h2 %>% filter(runID == min_runID)
...
p1 <- ggplot(data = example_data, aes(x = h, y = h.MWA)) +
      geom_point() +
      facet_grid(. ~ Heritability, labeller = "label_both") + 
      shared_theme +
