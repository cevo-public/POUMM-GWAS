...
png(filename = "figures/simulation_results_v2.png", width = linewidth, height = height, unit = "cm", res = 300)
...
```{r}
# Re-generate simulation tree, attach trait data to it
set.seed(1)
simulation_tree <- simulateHIVTreeExpBL(500)
for (idx in 501:550) {
        single_clade <- ape::extract.clade(phy = simulation_tree, node = idx)
        if (length(single_clade$tip.label) > 8 && length(single_clade$tip.label) < 30) {
                print(idx)
        }
}
single_clade <- ape::extract.clade(phy = simulation_tree, node = 516)

example_data <- example_simulation_results %>% rename("label" = "sample", "Heritability" = "H2")
example_data_lo_h2 <- example_data %>% filter(Heritability == 0.15)
example_data_hi_h2 <- example_data %>% filter(Heritability == 0.65)

# Plot trait values on a clade in the tree
mar <- 20
lo_h2_tree <- ggtree(single_clade) %<+% example_data_lo_h2 + 
        geom_tippoint(aes(color = v), size = 3) + 
        scale_color_viridis_b(name = "Pathogen\npart of trait") + 
        theme_tree(plot.margin=margin(0, mar, 0, mar))

hi_h2_tree <- ggtree(single_clade) %<+% example_data_hi_h2 + 
        geom_tippoint(aes(color = v), size = 3) + 
        scale_color_viridis_b(name = "Pathogen\npart of trait") + 
        theme_tree(plot.margin=margin(0, mar, 0, mar))
