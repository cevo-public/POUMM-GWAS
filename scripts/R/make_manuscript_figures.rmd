...
```{r}
p <- ggplot(data = traits, aes(x = trait, y = h.MWA)) +
      geom_point() +
      shared_theme +
...
# This is a super hacky way to get labels in between x ticks
# Hadley Wickam disapproves :/
...
x_axis_ticks[x_axis_ticks$CHROM == 22, "midpoint"] <-
        x_axis_ticks[x_axis_ticks$CHROM == 22, "CHR_start"] +
                (max(gwas_results_annotated[gwas_results_annotated$CHROM == 22, "BP_position"]) -
                        x_axis_ticks[x_axis_ticks$CHROM == 22, "CHR_start"]) / 2
x_axis_ticks_tmp <- tidyr::pivot_longer(
        data = x_axis_ticks,
        cols = c("midpoint", "CHR_start"),
        names_to = "marker",
        values_to = "tick_position") %>%
        mutate(CHROM = as.character(as.integer(CHROM)))
x_axis_ticks_tmp[seq(from = 2, to = 46, by = 2), "CHROM"] <- ""
x_axis_ticks_tmp[x_axis_ticks_tmp$CHROM == "23", "CHROM"] <- ""
x_axis_ticks_tmp[x_axis_ticks_tmp$CHROM == "24", "CHROM"] <- ""
...
overview_comparison <- ggplot(
        ...
        geom_point(aes(color = CHROM %% 2 == 0)) +
        scale_color_manual(values = c("grey", "black")) +
        scale_x_continuous(
                breaks = x_axis_ticks$tick_position,
        ...
        theme(
                ...
                axis.ticks.x = element_line(color = rep(c(NA, "black"), 23)),
...
p_vals_regional <- ggplot(
        ...
        geom_point(aes(color = -neg_log10_P_standard_minus_corrected)) +
        ...
        theme(
                ...
                axis.ticks.x = element_line(color = rep(c(NA, "black"), 23)),
                panel.grid.minor.x = element_blank(),
                plot.margin = unit(c(25, 5.5, 5.5, 5.5), "pt")) +
...
Debugging
```{r}
# Are trait values very different from what was inferred last time?
old_traits <- read.delim(file = "/Users/nadeaus/Polybox/200207_phenotypes.txt", sep = "", header = T)
trait_comparison <- merge(
        x = old_traits, y = traits,
        by.x = "IID", by.y = "id",
        all = T)

ggplot(data = trait_comparison, aes(x = total_spvl, y = trait)) + geom_point()  # calculated spVL indistinguishable
ggplot(data = trait_comparison, aes(x = corrected_spvl, y = h.MWA)) + geom_point()  # corrected spVL very similar, well correlated

# What about GWAS phenotypes file?
gwas_phenotypes <- read.delim(file = "output/gwas_phenotypes.txt", sep = "", header = T)
gwas_pheno_comparison <- merge(
        x = old_traits, y = gwas_phenotypes,
        by = "IID",
        all = T)
ggplot(data = gwas_pheno_comparison, aes(x = corrected_spvl, y = h.MWA)) + geom_point()  # also well correlated but previous GWAS had more samples?
```
