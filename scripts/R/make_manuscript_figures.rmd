...
example_simulation_results <- read.delim("output_revisions/2022-05-30_simulateFullDataset_2.txt")
...
```{r}
p1 <- ggplot(data = example_simulation_results %>% rename("Heritability" = H2), aes(x = h, y = h.MWA)) +
      geom_point() +
      facet_grid(. ~ Heritability, labeller = "label_both") + 
      shared_theme +
      labs(x = "Simulated host part of trait", y = ehat_name)
ggsave(plot = p1, filename = paste(FIGDIR, "example_simulation_trait_values.png", sep = "/"),
      width = linewidth * 2/3, height = height * 2/3, units = "cm")
```
